    ...
    <div className="relative group cursor-pointer" ref={dropdownRef}>
      ...
      {isDropdownOpen && (
        <div className="absolute py-4 px-4 right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg">
          <button className="py-2 mb-3 w-full border border-secondary-gray-700 rounded-lg bg-white text-secondary-gray-700 hover:bg-secondary-gray-100 hover:text-secondary-gray-700 focus:outline-none">
            Switch to Host
          </button>
          ...
          <ul>
            {userProfileDropdownLinks.map((menuItem, index) => (
              <li className="px-4 py-[8px] hover:bg-secondary-gray-100 place rounded-xl" key={index}>
                <a href="/" className="text-black text-md font-normal">
                  {menuItem}
                </a>
              </li>
            ))}
